...
The recommended way to test is browse to ```http://localhost:8080/docs``` and then authenticate with one of the provided users. If a new user wants to be created, find below the User **alice** and her credentials to be able to do so.
...
layer. We introduced the usage of **command pattern** to encapsulate the logic of each request performed against the API and from there, we can easily communicate the request between layers, keeping the responsabilities as separeted as possible. 
